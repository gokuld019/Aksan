...
export default function AwardsRecognition() {
  return (
    <section className="bg-white py-16 md:py-20" style={{ fontFamily: "'Noto Sans', sans-serif" }}>
      <div className="mx-auto max-w-6xl px-4 mt-20">
        {/* Heading */}
        <h2 className="text-center text-2xl md:text-[28px] font-bold text-[#132A5C] mb-3" style={{ fontFamily: "'Noto Sans', sans-serif" }}>
          Celebrated Equity and IPO Advisors
        </h2>

        <p className="text-center text-sm font-semibold tracking-wide text-[#E8622C] mb-3" style={{ fontFamily: "'Noto Sans', sans-serif" }}>
          List of Awards and Recognitions
        </p>

        <p className="mx-auto max-w-2xl text-center text-sm text-[#3A4A6B] leading-relaxed mb-12" style={{ fontFamily: "'Noto Sans', sans-serif" }}>
...
function AwardCard({ award }) {
  return (
    <div className="relative overflow-hidden rounded-md bg-[#0E1F45] shadow-md">
      {/* Photo */}
      <div className="relative h-56 w-full">
        ...
        {/* NSE badge, top-left */}
        <div className="absolute left-3 top-3 flex h-10 w-10 items-center justify-center rounded-full bg-[#F7931E] text-[10px] font-bold text-white shadow" style={{ fontFamily: "'Noto Sans', sans-serif" }}>
          NSE
        </div>

        {/* Dark gradient so the text block below reads cleanly */}
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#0E1F45] to-transparent" />
      </div>

      {/* Text block */}
      <div className="relative -mt-1 px-6 pb-6 pt-3 text-center">
        <h3 className="text-base font-semibold text-white" style={{ fontFamily: "'Noto Sans', sans-serif" }}>
          {award.company}
        </h3>
        <p className="mt-1 text-xs text-white/70" style={{ fontFamily: "'Noto Sans', sans-serif" }}>
          {award.date}
        </p>

        <button
          type="button"
          className="mt-4 rounded-full border border-white/40 px-6 py-1.5 text-xs font-medium tracking-wide text-white transition-colors hover:bg-white hover:text-[#0E1F45]"
